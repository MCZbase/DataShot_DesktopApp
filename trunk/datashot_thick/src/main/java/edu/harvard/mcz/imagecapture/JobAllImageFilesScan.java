		...
		public void run() {
			...
			if (startPoint.isDirectory() &&  (!startPoint.getName().equals("thumbs"))) { 
				...
				if (filesToThumbCount>0) { 
					...
					String runCommand = Singleton.getSingletonInstance().getProperties().getProperties().getProperty(ImageCaptureProperties.KEY_MOGRIFY_EXECUTABLE) 
							+ " -path thumbs -resize 80x120 " + filesToThumb.toString();
								...
								if (matches!=null && matches.size()==0) {
									...
									if (labelRead!=null) { 
										...
										try { 
											...
											ImagePreprocessError error =  new ImagePreprocessError(filename, "OCR Failed",
									...
									if (Singleton.getSingletonInstance().getBarcodeMatcher().matchesPattern(exifComment) 
										...
										if (exifComment.matches(ImageCaptureApp.REGEX_DRAWERNUMBER)) { 
											isDrawerImage = true;
									...
									if (matches==null) { 
										...
										ImagePreprocessError error =  new ImagePreprocessError(filename, "Bad Data",
